...
namespace Send
{
    class Program
    {
        ...
        public static async Task MainAsync(string[] args)
        {
            ...
            var rabbitSender = new RabbitMQSender(Configuration);

            


            var protoMessage = BuildMessage();
            Stream stream = new MemoryStream(protoMessage);
            var gm2 = ProtoBuf.Serializer.Deserialize<GoogleMessage2>(stream);

            await rabbitSender.SendAsync( profile:"proto", message: protoMessage);
